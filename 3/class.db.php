...
namespace vakata\database
{
    ...
    class sqlite_driver extends ADriver
    {
        protected $iid = 0;
        protected $aff = 0;
        ...
        public function __construct($settings) {
            ...
            if (is_file('./' . $this->settings->servername) && is_readable('./' . $this->settings->servername)) {
                $this->settings->database = './' . $this->settings->servername;
            }
        }
        public function connect() {
            if ($this->lnk === null) {
                try {
                    ...
                    throw new Exception('Connect error: ' . $this->lnk->lastErrorMsg());
                }
            }
            return true;
        }
        ...
        public function prepare($sql) {
            ...
            $temp = $this->lnk->prepare($sql);
            if (!$temp) {
                throw new Exception('Could not prepare : ' . $this->lnk->lastErrorMsg() . ' <'.$sql.'>');
            }
            return $temp;
        }
        public function execute($sql, $data = array()) {
            ...
            $rtrn = $sql->execute();
            if (!$rtrn) {
                throw new Exception('Prepared execute error : ' . $this->lnk->lastErrorMsg());
            }
        ...
        public function real_query($sql) {
            ...
            $temp = $this->lnk->query($sql);
            if (!$temp) {
                throw new Exception('Could not execute query : ' . $this->lnk->lastErrorMsg() . ' <'.$sql.'>');
            }
            $this->iid = $this->lnk->lastInsertRowID();
            $this->aff = $this->lnk->changes();
            return $temp;
        ...
        // TODO: not sure that correct for all reasons
        public function nf($result) {
